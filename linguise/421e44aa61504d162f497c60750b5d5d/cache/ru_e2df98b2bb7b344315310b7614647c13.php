...
function showMessageBox(title, message, onConfirm, onCancel = null) {
    ...
    if (!messageBox) {
        ...
        messageBox.className = 'custom-message-box no-translate';
        messageBox.setAttribute('data-no-translate', 'true');
        ...
        messageBox.innerHTML = `
            <div class="message-box-content">
                ...
                <div class="message-box-footer">
                    <button id="message-box-confirm" class="btn btn-primary">${translate('modals.confirmation.btn_confirm')}</button>
                    <button id="message-box-cancel" class="btn btn-outline" style="display:none;">${translate('modals.confirmation.btn_cancel')}</button>
                </div>
            </div>
        `;
...
function takePrize() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            // Show success modal
            showResultModal(translate('modals.result.title'), `
            ...
            // Show error modal
            showResultModal(translate('modals.error.title'), `
...
function makeAutomaticSelection() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            modalBody.innerHTML = `
                <i class="fas fa-clock" style="font-size: 3rem; color: #ff9f43; margin-bottom: 15px;"></i>
                <p>${translate('modals.timeout.message1')}</p>
                <p>${translate('modals.timeout.message2')}</p>
            `;
        }
                    ...
                    if (data.can_try_again && prizeType === 'low') {
                        ...
                        setTimeout(() => {
                            ...
                            showResultModalWithRetry(
                                ...
                                <p class="mt-3">${translate('modals.result.try_again_message')}</p>`,
                                prize
                            );
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const gameElements = document.querySelectorAll('.game-area, .modal, .card-result, .timer-container');
