<?php die(); ?><!DOCTYPE html><html lang="es" dir="ltr"><head>
    ...
    <title>Juego de recompensas diarias | M&#xF3;vil Digiminex</title>
...
<div class="daily-game-page">
    ...
    <div class="game-header">
        <h1>Juego de recompensa diario</h1>
        <p>&#xA1;Prueba tu suerte y gana las recompensas de USDT!</p>
        
        <div class="game-stats">
            <div class="stat-badge">
                <i class="fas fa-gamepad"></i>
                <span>Intentos restantes: <strong>87</strong></span>
            </div>
            <div class="stat-badge">
                <i class="fas fa-percentage"></i>
                <span>Ganar la oportunidad: <strong>80.0%</strong></span>
            </div>
        </div>
        
        <div class="vip-badge">
            <i class="fas fa-crown"></i> Nivel VIP: plata        </div>
    </div>
    ...
    <div class="game-area no-translate" data-no-translate="true">
        ...
        <div class="game-stage" id="direct-reward">
            <h2>Tu recompensa diaria</h2>
            <p>4.2 USDT listo! &#xBF;T&#xF3;malo ahora o prueba tu suerte por m&#xE1;s?</p>
            
            <div class="game-explanation">
                <div class="explanation-option">
                    <i class="fas fa-check-circle"></i>
                    <h3>Recompensa</h3>
                    <p>Obtenga garantizado 4.2 USDT ahora</p>
                </div>
                <div class="explanation-option">
                    <i class="fas fa-dice"></i>
                    <h3>Prueba tu suerte</h3>
                    <p>&#xA1;Riesga su recompensa garantizada por la oportunidad de ganar hasta 6 USDT!</p>
                </div>
            </div>
            
            <div class="decision-buttons"><i class="fas fa-check-circle"></i>
                    
                <button class="btn btn-success" onclick="takePrize()">Toma 4.2 USDT</button>
                 <i class="fas fa-dice"></i>
                    <button class="btn btn-warning" onclick="doubleOrNothing()">Prueba tu suerte</button>
            </div>
        </div>
        <!-- Card Selection (hidden initially) -->
        <div class="game-stage" id="card-selection" style="display: none;">
            <h2>Elige tu tarjeta</h2>
            <p>&#xA1;Seleccione una tarjeta para ver su recompensa!</p>
            
            <div class="possible-rewards">
                <span>Posibles recompensas:</span>
            ...
            <div id="timer-container" class="timer-container">
                ...
                <div class="timer-text">Hora de seleccionar una tarjeta: <span id="timer-seconds">30</span> segundos</div>
            </div>
    ...
    <div class="modal no-translate" id="resultModal" data-no-translate="true">
        <div class="modal-content">
            <div class="modal-header">
                <h3 id="resultModalTitle">&#xA1;Felicidades!</h3>
            ...
            <div class="modal-footer" id="resultModalFooter">
                <button onclick="closeModal()" class="btn btn-primary">DE ACUERDO</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal no-translate" id="countdownModal" data-no-translate="true">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>No seleccion&#xF3; una tarjeta dentro del l&#xED;mite de tiempo.</p>
                <p>La recompensa m&#xE1;s baja se ha agregado a su cuenta.</p>
            </div>
            <div class="modal-footer">
                <button onclick="window.location.reload()" class="btn btn-primary">DE ACUERDO</button>
            </div>
        </div>
    </div>
    ...
    <div class="custom-message-box no-translate" id="custom-message-box" data-no-translate="true" style="display: none;">
        <div class="message-box-content">
            <div class="message-box-header">
                <h3 id="message-box-title">Solicitud de confirmaci&#xF3;n</h3>
            </div>
            <div class="message-box-body">
                <p id="message-box-message">Si elige probar suerte, recibir&#xE1; la recompensa m&#xE1;s baja si sale del juego y su intento se contar&#xE1;. &#xBF;Quieres continuar?</p>
            </div>
            <div class="message-box-footer">
                <button id="message-box-confirm" class="btn btn-primary">Ok</button>
                 <button id="message-box-cancel" class="btn btn-outline">cancelar</button>
            </div>
        </div>
    </div>
...
function takePrize() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            // Show success modal with translated title and content
            showResultModal(
                translate('modals.result.title'), 
                `<i class="fas fa-trophy" style="font-size: 3rem; color: gold; margin-bottom: 15px;"></i>
                <h3>${gameState.reward} USDT</h3>
                <p>${translate('modals.result.message')}</p>`
            );
            ...
            // Show error modal with translated content
            showResultModal(
                translate('modals.error.title'), 
                `<i class="fas fa-exclamation-triangle" style="font-size: 3rem; color: #ea5455; margin-bottom: 15px;"></i>
                <h3>${translate('modals.error.generic_error')}</h3>
                <p>${data.message || translate('modals.error.try_again')}</p>`
            );
        }
    })
    .catch(error => {
        ...
        showResultModal(
            translate('modals.error.title'), 
            `<i class="fas fa-exclamation-triangle" style="font-size: 3rem; color: #ea5455; margin-bottom: 15px;"></i>
            <h3>${translate('modals.error.connection_error')}</h3>
            <p>${translate('modals.error.check_connection')}</p>`
        );
...
function doubleOrNothing() {
    ...
    // Show custom confirmation dialog with translations
    showMessageBox(
        translate('modals.confirmation.title'), 
        translate('modals.confirmation.try_luck_warning'),
        function() {
...
function makeAutomaticSelection() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            modalBody.innerHTML = `
                <i class="fas fa-clock" style="font-size: 3rem; color: #ff9f43; margin-bottom: 15px;"></i>
                <p class="no-translate" data-no-translate="true">${translate('modals.timeout.message1')}</p>
                <p class="no-translate" data-no-translate="true">${translate('modals.timeout.message2')}</p>
            `;
            ...
            if (modalTitle) modalTitle.textContent = translate('modals.timeout.title');
            if (modalButton) modalButton.textContent = translate('modals.timeout.btn_ok');
        }
...
function selectCard(cardId) {
        ...
        setTimeout(() => {
            ...
            if (data.status === 'success') {
                    ...
                    if (data.can_try_again && prizeType === 'low') {
                        ...
                        setTimeout(() => {
                            ...
                            showResultModalWithRetry(
                                ...
                                <p class="mt-3 no-translate" data-no-translate="true">${translate('modals.result.try_again_message')}</p>`,
                                prize
                            );
                        ...
                        setTimeout(() => {
                            ...
                            showResultModal(
                                ...
                                <h3>${prize.toFixed(2)} USDT</h3>
                                <p class="no-translate" data-no-translate="true">${translate('modals.result.message')}</p>`
                            );
                ...
                showResultModal(
                    ...
                    `<i class="fas fa-exclamation-triangle" style="font-size: 3rem; color: #ea5455; margin-bottom: 15px;"></i>
                    <h3>${translate('modals.error.generic_error')}</h3>
                    <p class="no-translate" data-no-translate="true">${data.message || translate('modals.error.try_again')}</p>`
                );
    ...
    .catch(error => {
        ...
        showResultModal(
            ...
            `<i class="fas fa-exclamation-triangle" style="font-size: 3rem; color: #ea5455; margin-bottom: 15px;"></i>
            <h3>${translate('modals.error.connection_error')}</h3>
            <p class="no-translate" data-no-translate="true">${translate('modals.error.check_connection')}</p>`
        );
...
function tryAgain() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            showResultModal(
                translate('modals.error.title'), 
                `<i class="fas fa-exclamation-triangle" style="font-size: 3rem; color: #ea5455; margin-bottom: 15px;"></i>
                <h3 class="no-translate" data-no-translate="true">${translate('modals.error.generic_error')}</h3>
                <p class="no-translate" data-no-translate="true">${data.message || translate('modals.error.try_again')}</p>`
            );
    ...
    .catch(error => {
        ...
        showResultModal(
            translate('modals.error.title'), 
            `<i class="fas fa-exclamation-triangle" style="font-size: 3rem; color: #ea5455; margin-bottom: 15px;"></i>
            <h3 class="no-translate" data-no-translate="true">${translate('modals.error.connection_error')}</h3>
            <p class="no-translate" data-no-translate="true">${translate('modals.error.check_connection')}</p>`
        );
...
// Ek önlem: Linguise tarafından dinamik içeriğin yeniden çevrilmesini engelleme
document.addEventListener('DOMContentLoaded', function() {
    // JSON içeriğini içeren tüm elementlere no-translate sınıfı ekle
    const gameElements = document.querySelectorAll('.game-area, .modal, .card-result, .timer-container, .modal-header h3, .message-box-header h3, .message-box-body p, #resultModalTitle');
    gameElements.forEach(el => {
        el.classList.add('no-translate');
        el.setAttribute('data-no-translate', 'true');
    });
    
    // Debug için - mevcut dil ve çeviriler
    console.log("Current language:", gameState.currentLanguage);
    console.log("Translation sample:", translate('modals.result.title'));
});
</script>
    ...
    <nav class="mobile-bottom-nav">
        ...
        <a href="/es/packages.php" class="nav-item ">
            ...
            <span data-i18n="buttons.packages">Paquetes</span>
                    ...
                    <a href="/es/daily-game.php" class="nav-item nav-item-main active">
                <div class="main-btn">
