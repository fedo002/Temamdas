...
function playGameStage($stage, $user_id, $card = null, $action = null, $amount = 0) {
    ...
    // Ödülleri veritabanından çek
    $prize = selectRandomPrize($vip_level, $stage);
    $win_amount = $prize['amount'];
    $win_chance = $prize['chance'] / 100; // Yüzdelik değere çevrildi
    $stake_amount = 0; // Eğer bir bahis sistemi olacaksa burası düzenlenmeli

    $stmt = $conn->prepare("INSERT INTO game_attempts (user_id, game_type, attempt_result, stake_amount, win_amount, stage, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
    ...
    if (!$stmt->bind_param('issddds', $user_id, $game_type, $result, $stake_amount, $win_amount, $stage, $win_chance)) {
